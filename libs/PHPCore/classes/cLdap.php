<?php
    // pull in web validation to make sure url is set correctly
    require_once( sCORE_INC_PATH . '/classes/cValidateWeb.php' );

    // pull in exception handling class
    ...
    class cLdap
    {
        ...
        public function __construct( array $aSpecConf, $sTag )
        {
            try
            {
                // check if we can look for the connection
                $bIsString = is_string( $sTag );
                if( empty( $aSpecConf )
                    || !isset( $aSpecConf[ 'ldap' ] )
                    || !is_array( $aSpecConf[ 'ldap' ] ) )
                {
                    // set vague message initially
                    $sErrMessage = 'The specified LDAP connection does not exist in ldap.xml.';

                    // set specific message if possible
                    if( $bIsString )
                    {
                        $sErrMessage = 'The specified LDAP connection "' . $sTag . '" does not exist in ldap.xml.';
                    }
                ...
                $aConnection = array();
                foreach( $aSpecConf[ 'ldap' ] as $vConnection )
                {
                    if( is_array( $vConnection )
                        && isset( $vConnection[ 'label' ] )
                        && is_string( $vConnection[ 'label' ] )
                        && $vConnection[ 'label' ] == $sTag )
                    {
                        $aConnection = $vConnection;
                        break;
                    }
                }

                // if we can't find the connection, get out
                ...
                if( !empty( $aConnection[ 'url' ] ) )
                {
                    // get a ValidateWeb object to check the url
                    $oValidateWeb = new cValidateWeb();

                    // make sure the url is a valid url
                    if( $oValidateWeb->ValidateURL( $aConnection[ 'url' ] ) === true )
                    {
                        $this->sUrl = $aConnection[ 'url' ];
                    }
                ...
                if( !empty( $aConnection[ 'dn' ] ) )
                {
                    // make sure the base directory is a string
                    if( is_string( $aConnection[ 'dn' ] ) )
                    {
                        $this->sDn = $aConnection[ 'dn' ];
                    }
                ...
                if( !empty( $aConnection[ 'port' ] ) && is_numeric( $aConnection[ 'port' ] ) )
                {
                    // only set the field if what is entered is non-emtpy and numeric
                    $this->iPort = $aConnection[ 'port' ];
                }

                // these 2 fields are optional, won't use ldap_bind without them...
                if( !empty( $aConnection[ 'bindrdn' ] ) && is_string( $aConnection[ 'bindrdn' ] ) )
                {
                    // only set the field if what's entered is non-empty and a string
                    $this->sBindRdn = $aConnection[ 'bindrdn' ];
                }

                if( !empty( $aConnection[ 'password' ] ) && is_string( $aConnection[ 'password' ] ) )
                {
                    // only set the field if what's entered is non-empty and a string
                    $this->sPassword = $aConnection[ 'password' ];
                }
            }
