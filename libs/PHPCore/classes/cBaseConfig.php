    ...
    class cBaseConfig
    {
        ...
        protected function ProcessIniKey( $sKey, $sValue, array &$aConfig )
        {
            try
            {
                ...
                if( strpos( $sKey, '.' ) !== false )
                {
                    ...
                    // check for an invalid key
                    else if ( !is_array( $aConfig[ $aPieces[ 0 ] ] ) )
                    {
        ...
        public function Write( $sFile, $vContents, $sExtension = null )
        {
            try
            {
                ...
                if( file_put_contents( $this->sConfigDirectory . DIRECTORY_SEPARATOR . $sFile, $sContents, LOCK_EX ) === false )
                {
        ...
        public function SaveHistoricalConfig( $sFile )
        {
            try
            {
                ...
                return file_put_contents( $this->sConfigDirectory . DIRECTORY_SEPARATOR . $sHistoryFile, $sOriginalContents, LOCK_EX ) !== false;
            }
            catch( Exception $oException )
            {
                throw cAnomaly::BubbleException( $oException );
        ...
        public function GetConfigFiles()
        {
            ...
            if( is_dir( $this->sConfigDirectory )
                && is_readable( $this->sConfigDirectory ) )
            {
                chdir( $this->sConfigDirectory );
                $aFiles = glob( '*.xml' );

                foreach( $aFiles as $iIndex => $sFile )
                {
                    if( strtolower( substr( $sFile, -1 ) ) !== 'l' )
                    {
                        unset( $aFiles[ $iIndex ] );
                    }
                }
