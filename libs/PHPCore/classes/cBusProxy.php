<?php
    // pull in the request factory
    require_once( sCORE_INC_PATH . '/classes/cRequest.php' );

    // pull in the HTTP status codes
    ...
    class cBusProxy
    {
        ...
        public function ExecuteGet( $sServiceURL, array $aArgs )
        {
            try
            {
                ...
                // perform the request to the service.
                $sResponse = $this->oRequest->Get( $sServiceURL );
                $iCode     = $this->oRequest->GetStatus();
        ...
        public function ExecutePost( $sServiceURL, array $aArgs )
        {
            ...
            {
                // perform the request to the service.
                $sResponse = $this->oRequest->Post( $sServiceURL, $aArgs );
                $iCode     = $this->oRequest->GetStatus();
        ...
        public function ExecutePut( $sServiceURL, array $aArgs, $sArgs )
        {
            try
            {
                if( !empty( $sArgs ) )
                {
                    ...
                    $sResponse = $this->oRequest->Put( $sServiceURL, $sArgs );
                    $iCode     = $this->oRequest->GetStatus();
                }
                else if( !empty( $aArgs[ 'PAYLOAD' ] ) )
                {
                    ...
                    {
                       // don't do anything.
                       $sResponse = $this->oRequest->Put( $sServiceURL, $sPayload );
                       $iCode     = $this->oRequest->GetStatus();
                    }
                    ...
                    else if( $sFormat == 'json' )
                    {
                        ...
                        if( $sArgs !== NULL )
                        {
                            $sResponse = $this->oRequest->Put( $sServiceURL, $sArgs, true );
                            $iCode     = $this->oRequest->GetStatus();
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    $sResponse = $this->oRequest->Put( $sServiceURL, $sPayload, true );
                    $iCode     = $this->oRequest->GetStatus();
                }
        ...
        public function ExecuteDelete()
        {
            ...
            {
                // perform the request to the service.
                $sResponse = $this->oRequest->Delete( $sServiceURL, $aArgs );
                $iCode     = $this->oRequest->GetStatus();
