<?php
    require_once( sCORE_INC_PATH . '/classes/cAnomaly.php' );        // get the error handling class
    require_once( sCORE_INC_PATH . '/classes/cArrayUtilities.php' ); // get the array utilities class
    ...
    class cXmlUtilities
    {
        ...
        public function ReadArrayFromFile( $sPath )
        {
            try
            {
                ...
                // fix for single array element inconsistency.
                if ( basename( $sPath ) !== 'app.xml' )
                {
                    $aModReturn = $aReturn;
                    foreach ( $aReturn as $sKey => $vValue )
                    {
                        if ( is_string( $sKey ) && !isset( $vValue[ 0 ] ) )
                        {
                            $aModReturn[ $sKey ] = array( $aModReturn[ $sKey ] );
                        }
                    }
                    $aReturn = $aModReturn;
                }
