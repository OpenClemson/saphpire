<?php
     ...
     * @version    0.6.0
     */
    class cDbMySql implements ifDb
    {
        /**
        ...
        protected function BindVariables( &$oResults, array $aBindVariables = array() )
        {
            try
            {
                ...
                foreach( $aBindVariables as $sVariable => $vValue )
                {
                    ...
                    {
                        throw new Exception( 'String not provided for bind variable name: ' . print_r( $sVariable, true ) );
                    }
        ...
        public function __construct( array $aSpecConf = array() )
        {
            try
            {
                ...
                if( !isset( $aSpecConf[ 'database' ] ) || empty( $aSpecConf[ 'database' ] ) )
                {
                    throw new Exception( 'Missing database setting.' );
                }
                $this->sDbName = $aSpecConf[ 'database' ];
                ...
                if( isset( $aSpecConf[ 'error-mode' ] ) && $aSpecConf[ 'error-mode' ] != '' )
                {
                    switch ( strtoupper( $aSpecConf[ 'error-mode' ] ) )
                    {
                        case "EXCEPTION":
        ...
        public function GetConnection()
        {
            try
            {
                ...
                if( !( $this->oConnection instanceof PDO ) )
                {
                    ...
                    if ( checkdnsrr( $this->sHost ) === false )
                    {
                    ...
                    if( !( $this->oConnection instanceof PDO ) )
                    {
                        throw new Exception( 'Could not connect to database host '
                                             ...
                                             . $this->sUsername . '. MySQL Error Message: ' . $sError );
                    }
                    else
                    {
                    ...
                    {
                        throw new Exception( 'Could not set error mode.' );
                    }
                    if( !$this->oConnection->setAttribute( PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC ) )
                    {
                        throw new Exception( 'Could not set fetch mode.' );
                    }
        ...
        public function RunQuery( $sQuery, array $aBindVariables = array() )
        {
            try
            {
                ...
                if( $oPrepared === false )
                {
                    throw new Exception( 'Could not prepare statement: ' . $sQuery . ' with bind variables: ' . print_r( $aBindVariables, true ) );
                }
                    ...
                    for( $i = 0; $i < $iVarCount; ++$i )
                    {
                        ...
                        if( $oPrepared->execute() === FALSE )
                        {
                            throw new Exception( 'Could not execute query: ' . $sQuery
                                                 ...
                                                 . ( ( bIS_CLI ) ? "\n" : '</br>' )
                                                 . print_r( $oPrepared->errorInfo(), true ) );
                        }
                    }
                }
                else
            ...
            try
            {
                ...
                if( $oResults->execute() === FALSE )
                {
                    throw new Exception( 'Could not execute query: ' . $sQuery
                                         ...
                                         . 'PDO Error information:'
                                         . ( ( bIS_CLI ) ? "\n" : '</br>' )
                                         . print_r( $oResults->errorInfo(), true ) );
                }
        ...
        public function ReturnColCount( $sQuery, array $aBindVariables = array() )
        {
            try
            {
                ...
                if( strtoupper( substr( $sQuery, 0, 6) ) != 'SELECT' )
                {
                    throw new Exception( 'ReturnColCount called without a SELECT statement.' );
                }
            ...
            try
            {
                ...
                else
                {
                    ...
                    if( preg_match( $sRegex, $sQuery, $aOutput) > 0 )
                    {
                        ...
                        if( $oStatement === false )
                        {
                            throw new Exception( 'Could not execute query: ' . "SELECT COUNT(*) FROM {$aOutput[ 1 ]}" );
                        }
        ...
        public function GetNextSequenceId( $sQuery = '' )
        {
            try
            {
                throw new Exception( 'GetNextSequenceId not supported with MySQL.' );
            }
            catch( Exception $oException )
            {
                throw cAnomaly::BubbleException( $oException );
        ...
        public function StartTransaction()
        {
            try
            {
                ...
                if( !empty( $this->sMyISAMTables ) )
                {
                    throw new Exception( 'The tables ' . $this->sMyISAMTables . ' do not allow transactions. StartTransaction cancelled.' );
                }
